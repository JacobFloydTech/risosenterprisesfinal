...
import {  GLTFLoader, SVGLoader, Water, DRACOLoader} from 'three/examples/jsm/Addons.js';
//@ts-ignore
...
export function addPointLight(scene: THREE.Scene) { 
    const light = new THREE.PointLight(undefined, 2.5, 350, 0);
...
export async function addMiddleGround(scene: THREE.Scene) { 
    return new Promise<void>((resolve, reject) => { 
        ...
        loader.load('/widerwider.glb', (obj) => { 
            ...
            const scale = 100;
            const instancedMesh = new THREE.InstancedMesh(child.geometry, new THREE.MeshStandardMaterial({map: texture}),20);
...
function getXPosition() {
    ...
    } while (randomNumber >= -80 && randomNumber <= 80)
    return randomNumber
}
...
export function LoadTree(scene: THREE.Scene) {
    const loader = new GLTFLoader();
    const count = 100;
    const positions: THREE.Vector3[] = Array.from({ length: count }).map(() => new THREE.Vector3(getXPosition(), -2, Math.random()*-1500));
    const dummy = new THREE.Object3D();
...
export function addCloud(scene: THREE.Scene) { 
    const loader = new GLTFLoader();
    loader.load('public/fluffy_cloud.glb', (obj) => { 
